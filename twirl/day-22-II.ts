...
export function solution1(input: string): number {
    ...
    const graph = createGraph(zBottoms, bricks.length);
    return bricks.reduce((sum, _, i) => sum + countFalling(graph, i), 0);
}

function createGraph(bottoms: Bottom[], total: number): Graph {
    ...
    for (const { index, on } of bottoms) {
        const brickDeps = res.get(index)!;
        for (const dep of on) {
            if (dep !== -1) {
                brickDeps.stands.add(dep);
                res.get(dep)!.supports.add(index);
            }
        }
    }
...
function countFalling(graph: Graph, index: number): number {
    const fallen = new Set<number>([index]);
    let rim = new Set<number>([index]);

    while (rim.size > 0) {
        const newRim = new Set<number>();
        for (const removedBlock of rim) {
            const deps = graph.get(removedBlock)!.supports;
            for (const supportedBlock of deps) {
                if (
                    !fallen.has(supportedBlock) &&
                    !newRim.has(supportedBlock)
                ) {
                    const pillars = graph.get(supportedBlock)!.stands;
                    let willFall = true;
                    for (const pillar of pillars) {
                        if (!fallen.has(pillar)) {
                            willFall = false;
                            break;
                        }
                    }
                    if (willFall) {
                        newRim.add(supportedBlock);
                    }
                }
            }
        }
        rim = newRim;
        for (const newFallenBlock of rim) {
            fallen.add(newFallenBlock);
        }
    }

...
function findTwoDigitNumberFromString(line: string): number {
    ...
    while (left < right) {
        const leftDigit = Number(line[left]);
        const rightDigit = Number(line[right]);
...
for await (const chunk of stream) {
    ...
    console.log(`🚀 ~ file: day-1.ts:47 ~ forawait ~ line:`, line);
    sum += findTwoDigitNumberFromString(line);
    console.log(`🚀 ~ file: day-1.ts:48 ~ forawait ~ sum:`, sum);
}

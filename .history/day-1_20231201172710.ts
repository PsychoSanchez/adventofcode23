...
function findTwoDigitNumberFromStringWithLetters(line: string): number | null {
  let left = 0,
    right = line.length;
  ...
  while (left <= right) {
    firstDigit ??= findDigit(left);
    secondDigit ??= findDigit(right);

    const isLeftDigitFound = firstDigit !== null;
    const isRightDigitFound = secondDigit !== null;

    if (isLeftDigitFound && isRightDigitFound) {
      return firstDigit * 10 + isRightDigitFound;
    }

...
function compareStringsBySlice({ line, fromIndex, compareTo }: { line: string; fromIndex: number; compareTo: string; }): boolean {

    // for (let i = 0; i < compareTo.length; i++) {
        // if (line[from + i] !== compareTo[i]) return false;
    // }
}

function findTwoDigitNumberFromStringWithLetters(line: string): number {
    // const firstDigit = DIGITS.filter(digit => line.includes(digit)).find;

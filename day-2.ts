...
import { unsafeParseInt } from "./shared/utils";
...
function colorStringToEnum(color: string): CubeType {
  switch (color) {
    case "red":
      return CubeType.Red;
    case "green":
...
const readInput = async () => {
  const text = await Bun.file("./data/day-2.data").text();
...
async function main1() {
  ...
  const possibleGames = input.filter((game) => {
    for (const set of game.sets) {
      ...
      if (red > MAX_NUM_RED || green > MAX_NUM_GREEN || blue > MAX_NUM_BLUE) {
        return false;
      }
    }
  ...
  const sum = possibleGames.reduce((acc, game) => acc + game.id, 0);
  return sum;
}

console.log(`The sum of possible game IDs is ${await main1()}`);
...
console.log(`The sum of possible game IDs is ${await main2()}`);

...
function getFileData() {
  return Bun.file("./data/day-7.data").text();
}
...
function getHandType(hand: Hand): HandType {
  const groupedCards: { [key: string]: number } = groupCards(hand);
  const countsArray = Object.values(groupedCards);
  ...
  if (countsArray.length === 2) {
    return countsArray.includes(4) ? HandType.FourOfAKind : HandType.FullHouse;
  }

  if (countsArray.length === 3) {
    return countsArray.includes(3) ? HandType.ThreeOfAKind : HandType.TwoPair;
  }
...
const RANKS = "AKQJT98765432".split("").reverse().join("");

function compareRankDesc(a: string, b: string): number {
  return RANKS.indexOf(b) - RANKS.indexOf(a);
}

function compareRankAsc(a: string, b: string): number {
...
async function main1() {
  ...
  for (const hands of handsByType) {
    hands.sort((a, b) => {
      for (let i = 0; i < 5; i++) {
        ...
        const rank = compareRankAsc(cardA, cardB);
        if (rank !== 0) {
          return rank;
        }
      }
  ...
  const winnings = handsByType.flat().reduce((acc, hand, index) => {
...
const r1 = await main1();
console.log(`Result: ${r1}`);
// assert.equal(r1, 6440);
...
const RANKS_WITHOUT_JOKER = "AKQJT98765432J".split("").reverse().join("");
const compareRankWithJokerAsc = (a: string, b: string): number => {
  if (a === "J" || b === "J") {
    if (a === b) {
      return 0;
    }
  ...
  return RANKS_WITHOUT_JOKER.indexOf(a) - RANKS_WITHOUT_JOKER.indexOf(b);
};
  ...
  const handsByType: Hand[][] = input
    .reduce(
      (acc: Hand[][], hand) => {
        ...
        assert(Array.isArray(acc[bumpedHandType]));

        acc[bumpedHandType]!.push(hand);
  ...
  for (const hands of handsByType) {
    hands.sort((a, b) => {
      for (let i = 0; i < 5; i++) {
        ...
        const rank = compareRankWithJokerAsc(cardA, cardB);
        if (rank !== 0) {
          return rank;
        }
      }
